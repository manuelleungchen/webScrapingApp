...
def check_Price(url, emailInput, targetPrice):
    ...
    if (convertedPrice < targetPrice):
        send_Mail(url, emailInput)

    driver.quit()

# Send Email Notification

def send_Mail(url, emailDes):

    fromaddr = "manuel.leungchen@gmail.com"  # Modify
    ...
    server = smtplib.SMTP('smtp.gmail.com', 587)  # Enter server
    server.starttls()
    server.login(fromaddr, "pzilxtqnyfcalfch")  # Hardcode email password
    text = msg.as_string()
